...
function callbackUrls(provider, merchantReference) {
  ...
  const base = `${origin}/api/writer/webinar-subscription/checkout`;
  ...
  return {
    ...
    cancelUrl: `${origin}/writer/webinars?webinar_subscription_payment=cancelled&purchase_ref=${encoded}`,
  };
}
...
async function loadWriter(writerUserId) {
  ...
  if (!String(writer.email || '').trim()) {
    const error = new Error(
      'Writer account requires an email address for payment checkout.'
    );
    error.status = 400;
    throw error;
  }
...
async function loadPlan(planId) {
  const plan = await getWebinarPlanById(planId, { activeOnly: true });

  if (!plan) {
    const error = new Error(
      'Selected webinar subscription plan was not found or is inactive.'
    );
    error.status = 404;
    throw error;
  }

  if (!moneyString(plan.price_usd) || decimalToCents(plan.price_usd) <= 0) {
    const error = new Error(
      'Selected webinar subscription plan does not have a valid paid price.'
    );
    error.status = 400;
    throw error;
  }

  if (!['monthly', 'yearly'].includes(String(plan.billing_cycle))) {
    const error = new Error('Unsupported webinar subscription billing cycle.');
    error.status = 400;
    throw error;
  }
...
async function loadPurchaseByReference(reference, connection = pool) {
  ...
  const [rows] = await connection.query(
    ...
    FROM webinar_subscription_purchases wsp
...
async function initializePaystack({ gateway, purchase, writer, plan }) {
  ...
  if (!data?.status || !data?.data?.authorization_url) {
    throw new Error(
      data?.message || 'Paystack did not return a checkout URL.'
    );
  }
  ...
  const data = await fetchJson(
    'https://api.flutterwave.com/v3/payments',
    {
      ...
      body: JSON.stringify({
        ...
        customizations: {
          title: 'Bloggad Webinar Plan',
          description: `${plan.name} webinar subscription`,
        },
        meta: {
          purpose: 'webinar_subscription',
          writer_user_id: Number(purchase.writer_user_id),
...
async function getWebinarSubscriptionCheckoutOptions() {
  return getReaderSubscriptionCheckoutOptions();
}

async function initializeWebinarSubscriptionPurchase({
  writerUserId,
  planId,
  ...
  const [result] = await pool.query(
    `
    INSERT INTO webinar_subscription_purchases (
      writer_user_id,
      plan_id,
      provider,
      gateway_mode,
  ...
  try {
    ...
    await pool.query(
      `
      UPDATE webinar_subscription_purchases
      SET
        provider_reference = ?,
...
async function activateVerifiedPurchase({
  purchase,
  verification,
}) {
  ...
  try {
    ...
    const [lockedRows] = await connection.query(
      `
      SELECT *
      FROM webinar_subscription_purchases
      WHERE id = ?
      LIMIT 1
      FOR UPDATE
      `,
    ...
    if (!locked) {
      throw new Error(
        'Webinar subscription purchase disappeared during verification.'
      );
    }
    ...
    if (!plan || String(plan.status) !== 'active') {
      throw new Error(
        'Webinar subscription plan is no longer active.'
      );
    }
    ...
    await connection.query(
      ...
      WHERE writer_user_id = ?
        AND status = 'active'
      `,
      [locked.writer_user_id]
    );

    const periodExpression =
      String(plan.billing_cycle) === 'yearly'
        ? 'DATE_ADD(NOW(), INTERVAL 1 YEAR)'
        : 'DATE_ADD(NOW(), INTERVAL 1 MONTH)';

    const [subscriptionResult] = await connection.query(
      `
      INSERT INTO webinar_subscriptions (
        writer_user_id,
        plan_id,
        purchase_id,
        status,
      ...
      VALUES (
        ?, ?, ?, 'active', NOW(), ${periodExpression}, ?, ?, ?, NOW(), NOW()
      )
      `,
      [
        locked.writer_user_id,
...
async function reconcileWebinarSubscriptionPurchase(
  ...
  if (!reference) {
    throw new Error(
      'Webinar subscription purchase reference is required.'
    );
  }
  ...
  if (!purchase) {
    throw new Error('Webinar subscription purchase not found.');
  }
  ...
  if (
    ...
    await pool.query(
      ...
      SET
        status = 'failed',
        gateway_response_json = ?,
        failure_reason = 'Verified payment did not match the expected webinar subscription purchase.',
        updated_at = NOW()
      WHERE id = ?
      `,
      [JSON.stringify(verification.raw || {}), purchase.id]
...
async function getWebinarSubscriptionPurchaseStatus({
  writerUserId,
  merchantReference,
}) {
...
async function markWebinarSubscriptionPurchaseCancelled(
  merchantReference
) {
