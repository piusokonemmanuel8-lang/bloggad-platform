    ...
    <article className={`bh-story-card ${post?.featured_image ? '' : 'bh-story-card-no-thumb'}`.trim()}>
      <div className="bh-story-main">
        <div className="bh-story-author">
          ...
          <Link className="bh-story-author-name" to={writerUrl(post)}>{author}<WriterVerificationBadge badge={post?.verification_badge} size={15} /></Link>
...
function StaffPick({ post }) {
  return (
    <Link className="bh-staff-pick" to={postUrl(post)}>
      <span className="bh-staff-author">{post?.writer_name || post?.website_name || 'Writer'}<WriterVerificationBadge badge={post?.verification_badge} size={14} /></span>
